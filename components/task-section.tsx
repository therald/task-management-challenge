...
export function TaskSection({ labels, tasks }: ITaskSection) {
  ...
  const tasksWithLabels = tasks.filter(({labels}) => !getValues('labelFilters')?.length ? true : labels.filter(({labelId}) => getValues('labelFilters')?.includes(labelId)).length).map(({ labels: taskLabels, ...task }) => ({
    ...task,
    labels: taskLabels?.map(({labelId}) => labels.find(({id}) => id === labelId)).filter(label => !!label) ?? []
  }))
            ...
            labels.map(label => (
              <DropdownMenuCheckboxItem 
                ...
                onCheckedChange={() => {
                  ...
                  setValue('labelFilters', currentLabels, { shouldDirty: true, shouldValidate: true, shouldTouch: true })
                }}
              >
                {label.title}
              </DropdownMenuCheckboxItem>
